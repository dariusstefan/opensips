...
#### get_nodes(cluster_id, proto) {#get-nodes-id}
...
#### free_nodes(nodes) {#free-nodes-id}
...
#### set_state(cluster_id, machine_id, state, proto) {#set-state-id}
...
#### check(cluster_id, sockaddr, server_id, proto) {#check}
...
#### send_to(cluster_id, protocol) {#send-to-id}
...
#### register_module(module_name, protocol, callback_function, timeout, auth_check, cluster_id) {#register_module-id}
