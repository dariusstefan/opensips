...
- add_rr_param() - see [add rr param id](#add-rr-param-id)
- check_route_param() - see 
				[check route param id](#check-route-param-id)
...
#### append_fromtag (integer) {#append-fromtag-id}
...
#### record_route() and record_route(string) {#record-route-id}
...
#### add_rr_param(param) {#add-rr-param-id}
...
Adds a parameter to the Record-Route URI (param must be in 
		...
		(see [record route id](#record-route-id)).
...
#### check_route_param(re) {#check-route-param-id}
...
The function checks if the URI parameters of the local Route 
		...
		(see [loose route id](#loose-route-id)).
...
The function checks the flow direction of the request.
		...
		parameter, the append_fromtag (see [append fromtag id](#append-fromtag-id) 
		module parameter must be enabled. Also this must be called only after 
		loose_route() (see [loose route id](#loose-route-id)).
...
The function checks the flow direction of the request 
		...
		[append fromtag id](#append-fromtag-id) module parameter 
