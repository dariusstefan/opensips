...
To create the dialog associated to an initial request, the flag 
	"dlg_flag" ([dlg flag id](#dlg-flag-id)) must be set before
...
The dialog is automatically destroyed when a "BYE" is 
	...
	- [default timeout id](#default-timeout-id)) and custom timeout (see 
	"timeout_avp" - [timeout avp id](#timeout-avp-id)). The 
...
The module is able to cut/terminate the call from the middle (proxy side)
	...
	"bye_on_timeout_flag" - 
	[bye on timeout flag id](#bye-on-timeout-flag-id) on dialog creation, BYEs will be
...
#### bye_on_timeout_flag (integer) {#bye-on-timeout-flag-id}
...
#### timeout_avp (string) {#timeout-avp-id}
...
#### default_timeout (integer) {#default-timeout-id}
