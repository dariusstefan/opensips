...
### Insertion modes {#sec-insertion-modes}
...
#### Insertion by Contact (default) {#sec-insertion-by-contact}
...
The mid-registrar will also append a parameter to each Contact URI
			...
			*[modparam contact match param](#modparam-contact-match-param)* module parameter)
...
#### Insertion by Path {#sec-insertion-by-path}
...
### Exported Parameters {#sec-modparam}


#### mode (integer) {#modparam-mode}


Working mode of the module.
			Refer to [sec working modes](#sec-working-modes) for
			more details.
...
- when a REGISTER is received, the script writer must call
			*[modfunc mid registrar save](#modfunc-mid-registrar-save)*
- the mid-registrar will insert itself on the call flow of
			all registrations according to the
			*[modparam insertion mode](#modparam-insertion-mode)*.
...
- *1 (Contact throttling mode)*
			...
			*[modparam outgoing expires](#modparam-outgoing-expires)* module
			...
			*[modparam outgoing expires](#modparam-outgoing-expires)*
...
#### insertion_mode (integer) {#modparam-insertion-mode}


SIP insertion mode of the module. Refer to
			[sec insertion modes](#sec-insertion-modes) for more details. Possible
			values are:
...
#### min_expires (integer) {#modparam-min-expires}
...
#### default_expires (integer) {#modparam-default-expires}
...
#### max_expires (integer) {#modparam-max-expires}
...
#### outgoing_expires (integer) {#modparam-outgoing-expires}
...
#### contact_match_param (string) {#modparam-contact-match-param}
...
#### default_q (integer) {#modparam-default-q}
...
Sets the default *"q"* value for new contacts.
		...
		*[modparam default q](#modparam-default-q)*
...
#### tcp_persistent_flag (string) {#modparam-tcp-persistent-flag}
...
#### realm_prefix (string) {#modparam-realm-prefix}
...
#### case_sensitive (integer) {#modparam-case-sensitive}
...
#### received_avp (string) {#modparam-received-avp}
...
#### received_param (string) {#modparam-received-param}
...
#### max_contacts (integer) {#modparam-max-contacts}
...
#### disable_gruu (integer) {#modparam-disable-gruu}
...
#### gruu_secret (string) {#modparam-gruu-secret}
...
### Exported Functions {#sec-modfunc}


#### mid_registrar_save(domain[, [flags][, [aor][, [outgoing_expires]]]]) {#modfunc-mid-registrar-save}
...
In Contact/AOR throttling modes (more info about working modes in [sec working modes](#sec-working-modes)),
...
Depending on the current working
			*[modparam mode](#modparam-mode)* and
			*[modparam insertion mode](#modparam-insertion-mode)*,
...
- in *"Contact throttling"* mode
  ...
  - append a parameter to each
								...
								*[modparam contact match param](#modparam-contact-match-param)*
...
- *outgoing_expires (optional, only relevant
		...
		request, which overrides the default
			*[modparam outgoing expires](#modparam-outgoing-expires)* module parameter.
...
#### mid_registrar_lookup(domain[, [flags][, [aor]]]) {#modfunc-mid-registrar-lookup}
...
- in *"Contact throttling"* mode
  ...
  - extract the *[modparam contact match param](#modparam-contact-match-param)*
