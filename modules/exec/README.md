...
#### time_to_kill (integer) {#time_to_kill}
...
#### exec(command, [stdin], [stdout], [stderr], [envavp]) {#exec_sync_exec}
...
WARNING - this function is deprecated and it will be remove in the next
		version - please use the exec() function (  [exec sync exec](#exec_sync_exec) ).
...
Executes an external command. This function does exactly the same as
		[exec sync exec](#exec_sync_exec) (in terms of input, output and processing),
