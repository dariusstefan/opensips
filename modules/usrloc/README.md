...
#### Contact matching {#contact-matching-algs}
...
#### Contact replication {#usrloc-replication}
...
In order to control the DB operations performed by the receiving instance(s),
		the **[skip replicated db ops](#skip_replicated_db_ops)**
...
- **1 (Write through) and 2 (Write back)**
					- default behaviour on all instances, **[skip replicated db ops](#skip_replicated_db_ops)** can be set
...
Configuring both receival and sending of usrloc replication packets is
		...
		**[accept replicated contacts](#accept_replicated_contacts)** and
		**[replicate contacts to](#replicate_contacts_to)** parameters of the
		module.
...
- *clusterer, if [replicate contacts to](#replicate_contacts_to)
...
#### matching_mode (integer) {#matching-mode}


What contact matching algorithm to be used. Refer to section
		[contact matching algs](#contact-matching-algs) for the description of the
		algorithms.
...
#### accept_replicated_contacts (integer) {#accept_replicated_contacts}
...
More details on the user location replication mechanism are available in [usrloc replication](#usrloc-replication)
...
#### replicate_contacts_to (integer) {#replicate_contacts_to}
...
#### skip_replicated_db_ops (int) {#skip_replicated_db_ops}
...
More details on the user location replication mechanism are available
		in [usrloc replication](#usrloc-replication)
...
#### max_contact_delete (int) {#max_contact_delete}
