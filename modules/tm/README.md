...
For how to define branch flags and use via script, see 
		[branch route](#branch-route) and the setbflag(), resetbflag() and
...
#### fr_inv_timeout (integer) {#fr_inv_timeout}
...
#### t_relay(proto:host:port,[flags]) {#trelay-1}
...
#### t_on_branch(branch_route) {#branch-route}
...
#### t_inject_branches(source[,flags]) {#t_inject_branches}
...
The function adds new SIP branches (destinations) to an existing 
		...
		the [t wait for new branches](#t_wait_for_new_branches) function for this).


The main usage scenario for this function (and also what makes it
		different from [t relay](#t_relay) is the ability to add new
...
#### t_wait_for_new_branches() {#t_wait_for_new_branches}
...
Normally if you have a transaction with two branches and 
		...
		[t inject branches](#t_inject_branches) function) until the fr_inv timer
		hits.
...
#### t_replicate(URI,[flags]) {#treplicate}
...
With each different
			...
			be equal to the
			**[fr timeout](#fr_timeout)** parameter.
...
With each different request received,
			...
			**[fr inv timeout](#fr_inv_timeout)** parameter.


*"$T_fr_inv_timeout = NULL;"* will reset it to
			**[fr inv timeout](#fr_inv_timeout)**.
