...
**update_stat("packets:$var(ptype)", "+1")**).
		...
		using the **[modparam stat groups](#modparam-stat-groups)**
		module parameter.


The module allows easy iteration over the statistics of a group using
		the **[modfunc stat iter init](#modfunc-stat-iter-init)**
		and **[modfunc stat iter next](#modfunc-stat-iter-next)**
		functions.
...
#### stat_groups (string) {#modparam-stat-groups}
...
#### stat_iter_init(group, iter) {#modfunc-stat-iter-init}
...
#### stat_iter_next(name, val, iter) {#modfunc-stat-iter-next}
