...
#### rest_get(url, body_pv[, [ctype_pv][, [retcode_pv]]]) {#rest_get}
...
#### rest_post(url, send_body_pv, [send_ctype_pv], recv_body_pv[, [recv_ctype_pv][, [retcode_pv]]]) {#rest_post}
...
#### rest_put(url, send_body_pv, [send_ctype_pv], recv_body_pv[, [recv_ctype_pv][, [retcode_pv]]]) {#rest_put}
...
#### rest_append_hf(txt) {#rest_append_hf}
...
Sends a POST HTTP request. This function behaves exactly the same as
		**[rest post](#rest_post)** (in terms of input, output and processing),
		but in an asynchronous way. Script execution is suspended until the
		entire content of the HTTP response is available.
...
Sends a PUT HTTP request. This function behaves exactly the same as
		**[rest put](#rest_put)** (in terms of input, output and processing),
		but in an asynchronous way. Script execution is suspended until the
		entire content of the HTTP response is available.
