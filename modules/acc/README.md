...
To account a transaction and to choose which set of backends to be
		...
		accounting by using the [do accounting](#do_accounting) script function.
...
In order to have CDRs accounting, first you need to set the
			*cdr* flag when calling
			[do accounting](#do_accounting) script function for the
...
### Exported Parameters {#ACC-param-id}
...
#### do_accounting(type, flags, table) {#do_accounting}
...
#### drop_accounting([type], [flags]) {#drop_accouting}
