...
- Homer encapsulation protocl mode (HEP v1/2/3). With version 2.2
			...
			[hep set id](#hep_set_id) for more details. If you want more
...
#### hep_set([data_type,] chunk_id, [vendor_id,] chunk_data) {#hep_set_id}
...
- *data_type(string)* - same meaning as in
			[hep set id](#hep_set_id); can miss if it's a generic chunk
- *chunk_id* - same meaning as in
			[hep set id](#hep_set_id)
...
- *chunk_data_pv(writable pvar)* - will hold the data inside the
  ...
  - 0x0002  proto_id(string) - see [hep set id](#hep_set_id) for possible values
  ...
  - 0x000B  proto_type(string) - see [hep set id](#hep_set_id) for possible values
