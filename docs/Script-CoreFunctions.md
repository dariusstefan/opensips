...
Only works if [enable_asserts](https://web.opensips.org/docs/manual/3-1/script-coreparameters#toc53) is set to *true*. If the given expression evaluates to *false*, script execution is stopped and the [error_route](https://web.opensips.org/docs/manual/3-1/script-routes#toc5) is executed. If [abort_on_assert](https://web.opensips.org/docs/manual/3-1/script-coreparameters#toc24) is enabled, OpenSIPS will also shutdown.
...
## cache_fetch(storage_id, attribute, result_pv) {#cache_fetch}
...
## cache_counter_fetch(storage_id, counter_attribute, result_pv) {#cache_counter_fetch}
...
## cache_add( storage_id, attribute, increment, expire, [new_val_pv]) {#cache_add}
...
## cache_sub(storage_id, attribute, decrement, expire, [new_val_pv]) {#cache_sub}
...
## cache_raw_query(storage_id, raw_query, result_avps) {#cache_raw_query}
