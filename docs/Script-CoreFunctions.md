...
Only works if [enable_asserts](https://web.opensips.org/docs/manual/3-0/script-coreparameters#toc53) is set to *true*. If the given expression evaluates to *false*, script execution is stopped and the [error_route](https://web.opensips.org/docs/manual/3-0/script-routes#toc5) is executed. If [abort_on_assert](https://web.opensips.org/docs/manual/3-0/script-coreparameters#toc24) is enabled, OpenSIPS will also shutdown.
...
## cache_add( storage_id, attribute_str, increment_value, expire[, new_val_pvar]) {#cache_add}
