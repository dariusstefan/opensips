...
Only works if [enable_asserts](https://web.opensips.org/docs/manual/2-2/script-coreparameters#toc53) is set to *true*. If the given expression evaluates to *false*, script execution is stopped and the [error_route](https://web.opensips.org/docs/manual/2-2/script-routes#toc5) is executed. If [abort_on_assert](https://web.opensips.org/docs/manual/2-2/script-coreparameters#toc24) is enabled, OpenSIPS will also shutdown.
...
For more see [Flags Documentation](https://web.opensips.org/docs/manual/2-4/script-flags).
...
For more about script flags, see [Flags Documentation](https://web.opensips.org/docs/manual/2-4/script-flags).
...
## raise_event(string[, avp[, avp ] ] ) {#raise_event}
...
Replaced as functionality by the [`$log_level`](Script-CoreVar.md#log_level) script variable.
