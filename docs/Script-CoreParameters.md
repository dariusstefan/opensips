...
Only relevant when the HP_MALLOC compile flag is enabled. If set to "on", on each startup, OpenSIPS will attempt to restore the memory fragmentation pattern it had before the stop/restart. If no [pattern_file](https://web.opensips.org/docs/manual/1-11/script-coreparameters#toc67) from a previous run is found, memory warning is skipped, and the memory allocator simply starts with a big chunk of memory.
...
How much of OpenSIPS's memory should be fragmented with the pattern of the previous run, upon a restart. Used at startup, if [mem_warming](https://web.opensips.org/docs/manual/1-11/script-coreparameters#toc65) is enabled.
...
The memory fragmentation pattern of a previous OpenSIPS run. Used at startup, if [mem_warming](https://web.opensips.org/docs/manual/1-11/script-coreparameters#toc65) is enabled.
...
On top of RFC 5923's connection reusage (aliasing) mechanism, TCP connections in OpenSIPS are also persistent across multiple SIP dialogs. This can be controlled with the [tcp_connection_lifetime](https://web.opensips.org/docs/manual/1-11/script-coreparameters#toc94) global parameter.

  

> [!WARNING]
> Enabling the global **tcp_accept_aliases** parameter (RFC 5923) for end-user initiated connections (who are most likely grouped by one or more public IPs) is an open vector for call hijacking! In such platforms, we recommend using the [force_tcp_alias()](https://web.opensips.org/docs/manual/1-11/script-corefunctions#toc16) core function, in order to employ RFC 5923 behaviour only in conjunction with adjacent SIP proxies.
...
Only relevant when [log_stderror](https://web.opensips.org/docs/manual/1-11/script-coreparameters#toc62) is set to *true*. Enables the use of the [color escape sequences](https://web.opensips.org/docs/manual/1-11/script-corevar#toc97), otherwise they will have no effect.
