...
Only relevant if [asserts](https://web.opensips.org/docs/manual/2-2/script-corefunctions#toc2) are enabled. Set to *true* in order to make OpenSIPS shut down immediately in case a script assert fails.
...
Set to *true* in order to enable the [assert](https://web.opensips.org/docs/manual/2-2/script-corefunctions#toc2) script statement.
...
Value of 'log_level' parameter can also be get and set dynamically using [log_level](Interface-CoreMI.md#log_level) Core MI function or [`$log_level`](Script-CoreVar.md#log_level) script variable.
...
Only relevant when the HP_MALLOC compile flag is enabled. If set to "on", on each startup, OpenSIPS will attempt to restore the memory fragmentation pattern it had before the stop/restart. If no [pattern_file](https://web.opensips.org/docs/manual/2-2/script-coreparameters#toc68) from a previous run is found, memory warning is skipped, and the memory allocator simply starts with a big chunk of memory, like all other allocators.
...
How much of OpenSIPS's memory should be fragmented with the pattern of the previous run, upon a restart. Used at startup, if [mem_warming](https://web.opensips.org/docs/manual/2-2/script-coreparameters#toc66) is enabled.
...
The memory fragmentation pattern of a previous OpenSIPS run. Used at startup, if [mem_warming](https://web.opensips.org/docs/manual/2-2/script-coreparameters#toc66) is enabled.
...
> [!WARNING]
> Enabling the global **tcp_accept_aliases** parameter (RFC 5923) for end-user initiated connections (who are most likely grouped by one or more public IPs) is an open vector for call hijacking! In such platforms, we recommend using the [force_tcp_alias()](https://web.opensips.org/docs/manual/2-2/script-corefunctions#force_tcp_alias) core function, in order to employ RFC 5923 behaviour only in conjunction with adjacent SIP proxies.
...
*Default value: 0 (not set). Setting [tcp_keepcount](Script-CoreParameters.md#toc99) to any value will enable [tcp_keepalive](Script-CoreParameters.md#toc98).*
...
*Default value: 0 (not set). Setting [tcp_keepidle](Script-CoreParameters.md#toc100) to any value will enable [tcp_keepalive](Script-CoreParameters.md#toc98).*
...
*Default value: 0 (not set). Setting [tcp_keepinterval](Script-CoreParameters.md#toc101) to any value will enable [tcp_keepalive](Script-CoreParameters.md#toc98).*
...
Only relevant when [xlog](https://web.opensips.org/docs/manual/2-2/script-corefunctions#toc57) is set to *true*. Enables the use of the [color escape sequences](https://web.opensips.org/docs/manual/2-2/script-corevar#toc98), otherwise they will have no effect.
...
Default value for the logging level of the [xlog](https://web.opensips.org/docs/manual/2-2/script-corefunctions#toc57) core function, when the log_level parameter is omitted.
