...
Only relevant if [asserts](https://web.opensips.org/docs/manual/2-4/script-corefunctions#toc2) are enabled. Set to *true* in order to make OpenSIPS shut down immediately in case a script assert fails.
...
Set to *true* in order to enable the [assert](https://web.opensips.org/docs/manual/2-4/script-corefunctions#toc2) script statement.
...
Only relevant when the HP_MALLOC compile flag is enabled. If set to "on", on each startup, OpenSIPS will attempt to restore the memory fragmentation pattern it had before the stop/restart. If no [pattern_file](https://web.opensips.org/docs/manual/2-4/script-coreparameters#toc68) from a previous run is found, memory warming is skipped, and the memory allocator simply starts with a big chunk of memory, like all other allocators.
...
How much of OpenSIPS's memory should be fragmented with the pattern of the previous run, upon a restart. Used at startup, if [mem_warming](https://web.opensips.org/docs/manual/2-4/script-coreparameters#toc66) is enabled.
...
Only relevant when [xlog](https://web.opensips.org/docs/manual/2-4/script-corefunctions#toc57) is set to *true*. Enables the use of the [color escape sequences](https://web.opensips.org/docs/manual/2-4/script-corevar#toc98), otherwise they will have no effect.
...
Default value for the logging level of the [xlog](https://web.opensips.org/docs/manual/2-4/script-corefunctions#toc57) core function, when the log_level parameter is omitted.
