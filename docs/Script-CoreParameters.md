...
Only relevant if [asserts](https://web.opensips.org/docs/manual/4-0/script-corefunctions) are enabled. Set to *true* in order to make OpenSIPS shut down immediately in case a script assert fails.
...
Set to *true* in order to enable the [assert](https://web.opensips.org/docs/manual/4-0/script-corefunctions) script statement.
...
How much of OpenSIPS's memory should be fragmented with the pattern of the previous run, upon a restart. Used at startup, if [mem_warming](https://web.opensips.org/docs/manual/4-0/script-coreparameters) is enabled.
...
Only relevant when [xlog](https://web.opensips.org/docs/manual/4-0/script-corefunctions) is set to *true*. Enables the use of the [color escape sequences](https://web.opensips.org/docs/manual/4-0/script-corevar), otherwise they will have no effect.
