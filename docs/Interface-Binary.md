...
In order to listen for incoming Binary Packets, a **bin:** interface must be specified.  Its number of listener processes can be tuned with *[tcp_workers](https://opensips.org/Documentation/Script-CoreParameters-4-1#tcp_workers)* core parameter.
...
More details can be found in the [dialog](https://www.opensips.org/html/docs/modules/4.1.x/dialog#dialog-clustering) and [usrloc](https://www.opensips.org/html/docs/modules/4.1.x/usrloc#distributed-sip-user-location) documentation pages.
...
* *int bin_send(union sockaddr_union *de*[tcp_workers](https://opensips.org/Documentation/Script-CoreParameters-4-1#tcp_workers) *core parameter.st)* - sends the Binary Packet to a given destination over UDP
