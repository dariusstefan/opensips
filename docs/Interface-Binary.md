...
In order to listen for incoming Binary Packets, a **bin:** interface must be specified.  Its number of listener processes can be tuned with *[tcp_workers](https://web.opensips.org/docs/manual/devel/script-coreparameters#tcp_workers)* core parameter.
...
* *int bin_send(union sockaddr_union *de*[tcp_workers](https://web.opensips.org/docs/manual/devel/script-coreparameters#tcp_workers) *core parameter.st)* - sends the Binary Packet to a given destination over UDP
