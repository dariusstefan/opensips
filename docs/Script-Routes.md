...
The error route is executed automatically when a parsing error occurs during SIP request processing, or when a script [assert](http://www.opensips.org/Documentation/Script-CoreFunctions-3-5#toc2) fails. It allows the administrator to decide what to do in such error cases.
...
**Triggered by** : the [event_route](http://www.opensips.org/html/docs/modules/3.5.x/event_route) module when an event raised by the OpenSIPS Event Interface
