...
A route can return a set of values, that can later be retrieved from the route's calling context using the [`$return`](https://www.opensips.org/Documentation/Script-CoreVar-4-1#return) variable.
...
## branch_route  [🔗](#branch_route) {#branch_route}
...
## failure_route  [🔗](#failure_route) {#failure_route}
...
## onreply_route  [🔗](#onreply_route) {#onreply_route}
...
## error_route  [🔗](#error_route) {#error_route}

The error route is executed automatically when a parsing error occurs during SIP request processing, or when a script [assert](http://www.opensips.org/Documentation/Script-CoreFunctions-4-1#toc2) fails. It allows the administrator to decide what to do in such error cases.
...
## local_route  [🔗](#local_route) {#local_route}
...
## startup_route  [🔗](#startup_route) {#startup_route}
...
## timer_route  [🔗](#timer_route) {#timer_route}
...
**Triggered by** : the [event_route](http://www.opensips.org/html/docs/modules/4.1.x/event_route) module when an event raised by the OpenSIPS Event Interface
