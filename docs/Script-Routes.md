...
The error route is executed automatically when a parsing error occurs during SIP request processing, or when a script [assert](https://web.opensips.org/docs/manual/3-4/script-corefunctions) fails. It allows the administrator to decide what to do in such error cases.
