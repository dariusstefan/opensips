...
A route can return a set of values, that can later be retrieved from the route's calling context using the [`$return`](https://web.opensips.org/docs/manual/devel/script-corevar#return) variable.
...
The error route is executed automatically when a parsing error occurs during SIP request processing, or when a script [assert](https://web.opensips.org/docs/manual/devel/script-corefunctions#toc2) fails. It allows the administrator to decide what to do in such error cases.
