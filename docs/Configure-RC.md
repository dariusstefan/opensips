...
The file contains control options for interacting with the database, for run-time interaction with OpenSIPS ( eg. via the [MI interface](https://web.opensips.org/docs/manual/1-9/interface-mi) ) and also some control options for the provisioning done via the opensipsctl and osipsconsole tools.
