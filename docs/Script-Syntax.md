...
* *string* (unlimited size)
  * note that some functions which use strings may have internal buffers which limit the maximum size of the strings (e.g. the [xlog()](https://web.opensips.org/docs/manual/3-0/script-corefunctions#toc54) function's output buffer is configurable via [xlog_buf_size](https://web.opensips.org/docs/manual/3-0/script-coreparameters#toc96))
* *double* (packed as string), through the **[mathops](../modules/mathops/README.md)** module

### Complex

* *list* via the **[`$avp` variable](https://web.opensips.org/docs/manual/3-0/script-corevar#toc2)**
...
[Module functions](https://web.opensips.org/docs/manual/3-0/function-index) become available for OpenSIPS script writers to use once their respective modules are loaded via the **loadmodule** statement (see the "Script Format" section above).  All module functions implement the same internal function interface, such that they benefit from the following, more sophisticated calling convention:
