...
* *string* (unlimited size)
  * note that some functions which use strings may have internal buffers which limit the maximum size of the strings (e.g. the [xlog()](https://web.opensips.org/docs/manual/3-6/script-corefunctions#toc54) function's output buffer is configurable via [xlog_buf_size](https://web.opensips.org/docs/manual/3-6/script-coreparameters#toc96))
* *double* (packed as string), through the **[mathops](../modules/mathops/README.md)** module

### Complex

* *list* via the **[`$avp` variable](https://web.opensips.org/docs/manual/3-6/script-corevar#toc2)**
...
All OpenSIPS [core](https://web.opensips.org/docs/manual/3-6/script-corefunctions) and [module](https://web.opensips.org/docs/manual/3-6/function-index) functions internally share the same function interface, such that they benefit from the following calling convention:
